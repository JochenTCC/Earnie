...
- **Credentials & persistence**: Loxone and HA secrets go through `.env` (`runtime_store/dotenv_io.py` `write_loxone_dotenv()` / `write_ha_dotenv()`). OpenEMS still uses `config.json["ehal"].openems`. Non-secret HA settings (`ehal.backend`, `ehal.ha.sign`) stay in `config.json` via `save_main_config`. The SB page calls these existing forms; it does not reimplement them.
...
- **HA add-on self-discovery via Supervisor proxy.** Inside the Supervisor add-on network, mDNS multicast often fails (no `host_network`). With `homeassistant_api: true`, `discover_home_assistant()` prefers a `SUPERVISOR_TOKEN` probe of `http://supervisor/core/api/` (`integrations/ha_supervisor.py`, method `supervisor`) before falling through to mDNS. Runtime auth resolves empty `EHAL_HA_TOKEN` (`.env`) to `SUPERVISOR_TOKEN` and empty `EHAL_HA_BASE_URL` to `http://supervisor/core` when `EARNIE_INSTALL_CONTEXT=homeassistant_addon`.
...
- **Optimierer-Dienst/EHAL-Com hiding applies unconditionally, not just during onboarding.** `ui/navigation.py::_echtzeit_page_specs()` has an `include_daemon_pages` flag; `_append_konfiguration_and_echtzeit()` now derives it from `ui.setup_readiness.needs_sb_setup()` directly whenever daemon pages would otherwise show (`show_daemon`), rather than only when `force_echtzeit` is true (the greenfield-onboarding forced-nav path in `_restricted_page_specs`).
  - This closes a previously known gap: `runtime_store/dotenv_io.py::loxone_setup_deferred()` intentionally stays "deferred" (blocking pre-nav gate does **not** fire) for a *mature* config (`needs_planning_onboarding()` false, e.g. non-empty `flexible_consumers`) that has **never** had any backend credentials configured — confirmed by the existing, deliberate test `tests/test_dotenv_io.py::test_require_loxone_credentials_for_prod_without_onboarding`. The blocking gate alone therefore doesn't cover that state; nav-level gating now does, independently of the gate.
  - **Test-hygiene pass done alongside this:** several nav-visibility tests (`tests/test_ui_navigation.py`) previously passed only because real Loxone credentials happened to be loaded from a developer's local `.env` (`tests/conftest.py`'s `load_dotenv`), not because they asserted the right thing — the gating was a no-op outside onboarding before this change, so nothing exercised the credential-dependent branch. They now set `LOXONE_IP`/`LOXONE_USER`/`LOXONE_PASS` explicitly via `monkeypatch.setenv`, mirroring the existing pattern in `tests/test_loxone_connectivity.py::TestLoxoneEnvHelpers`, so they're deterministic regardless of the machine running them.
...
- **Greenfield landing page depends on `live_environment`.** `ui/navigation.py::_restricted_page_specs()` previously always defaulted to Hauskonfigurator (`house_config_default=True`, unchanged since before the SB page existed). Now, when `live_environment` is among the active `EARNIE_UI_MODES`, Smarthome-Backend is the default page instead: on a real install (LoxBerry/HA add-on/manual with real hardware) the backend connection is the prerequisite for everything else, so it makes sense to land there first rather than on Hauskonfigurator. Without `live_environment` (e.g. `scenario_explorer`-only planning/demo setups with no real hardware expected — this is also the Community-Cloud-demo default), Hauskonfigurator stays the landing page, matching the documented Cloud-Demo behaviour ("Start im Hauskonfigurator", `docs/einrichtung/betrieb.md`/`private-env.md`).
