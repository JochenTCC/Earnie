...
**Implemented (2026-08-14):** `share/config/tariffs.json` now has a `settlement_mtu` field (`"15min"` default / `"60min"`), validated by `share/config/tariffs.schema.json` and normalized through `house_config/tariffs_store.py::_normalize_dach_fields`. When a tariff is `"60min"`, `data/backtesting_prices.py::import_brutto_cent_for_slots` and `data/feed_in_prices.py::enrich_matrix_feed_in_prices` replace each QH EPEX value with the arithmetic mean of its clock hour (`data/market_prices.py::hourly_settlement_epex_values`) before applying the tariff's markup formula — matching the EPEX SDAC "average rule" (the exchange's own 60-min index price is by construction the mean of the four 15-min clearing prices; see backlog research in `backlog/Backlog.md`). Applies identically to Live and SE (§3.2).

**Full pass (2026-08-14):** every `spot_hourly` tariff in the catalog was researched individually (official Preisblatt/Tarifblatt/product page where reachable) and now carries an explicit `settlement_mtu` plus a `notes` citation — the previous "assume `15min` default when unknown" approach was explicitly rejected as unsafe (several suppliers, e.g. `at_pullstrom_pullstrom_ora`, `at_smartenergy_smartcontrol`, `at_wuesterstrom_wuesterstrom_spot`, kept genuine hourly billing after the Oct-2025 EPEX MTU switch despite offering a "spot" product). Two ids remain genuinely unresolved after a real search attempt and are flagged in their own `notes` rather than silently defaulted:

| Settlement | Catalog ids | Evidence pattern |
|------------|-------------|-------------------|
| **`"60min"` — genuine hourly billing** | `awattar_at`, `de_awattar_de_hourly_de`, export `dynamic_epex` (SUNNY Spot), `at_pullstrom_pullstrom_ora`, `at_smartenergy_smartcontrol`, `at_wuesterstrom_wuesterstrom_spot`, `at_wien_energie_optima_voll_aktiv`, `at_stadtwerke_klagenfurt_ekg_strom_flex`, `at_energie_steiermark_first_energy_steirerstrom_spot`, export `at_smartenergy_smartsun_spot` | Preisblatt/product page explicitly says "stündlich" / one price per clock hour, even where 15-min smart-meter (IME) readout is required |
| **`"15min"` — native EPEX-QH pass-through** | `at_tiwag_flex_privat`, `at_aae_naturstrom_naturstrom_spot_stunde_ii` (despite "Stunde" in the name — invoice display is averaged, the underlying charge formula is per QH), `at_oekostrom_oeko_spot`, `at_spotty_smart_active`, `at_elektrizitaetswerk_perg_heimatstrom_spot`, `at_hofer_gruenstrom_hofer_gruenstrom_spot`, `at_verbund_v_strom_spot`, `at_kittel_muehle_aquavento_flex_smart`, `at_avia_energy_avia_strom_plus_stundenfloater` (despite "Stundenfloater" in the id — product was renamed "15min FLOATER" and switched to QH billing), `at_energie_ag_oberoesterreich_oekostrom_spot`, `at_wels_strom_privat_strom_spot`, `at_vkw_strom_dynamisch`, `at_vkw_pv_dynamisch`, `de_tibber_tibber_dynamic` | Preisblatt/product page explicitly says "viertelstündlich" / "96 Preise pro Tag" |
| **`"15min"`, Medium confidence** | `de_eprimo_eprimo_smart` | Multiple consistent secondary sources describe 15-min-Takt billing; no primary-source quote could be pulled (eprimo.de is a JS-rendered SPA not readable via fetch) — flagged in `notes`, follow-up recommended |
| **Unresolved — left without `settlement_mtu` (implicit default `"15min"`), flagged in `notes`** | `at_schlau_pv_gmbh_spot` (site in maintenance mode, no reachable Preisblatt), `at_energie_steiermark_e1_floater` (no such Energie-Steiermark product found; likely supplier/id mismatch — the "E1 Floater" name traces to an unrelated Swiss company, First Energy AG) | Real search attempts made; genuinely inconclusive |

CH catalog entries (`ch_ekz_elektrizitaetswerke_des_kantons_zuerich_dynamischer_tarif`, `ch_groupe_e_vario`) are set to **`"60min"`** — not because the retail product is billed hourly (both EKZ and Groupe E do change customer-facing prices every 15 minutes), but because the underlying EPEX-CH day-ahead auction itself has not switched to 15-min MTU yet (planned for **2026-11-03**, per EPEX SPOT; Swiss suppliers running quarter-hourly retail prices today are doing so via a proprietary/grid-load-based engine, not EPEX-CH pass-through). Earnie's own CH price source (Energy-Charts CH) is still hourly-only (§3.1), so there is no QH EPEX-CH series to average or pass through either way — `"60min"` reflects the data that is actually available. Revisit after 2026-11-03 once Energy-Charts CH may start publishing native QH prices.
