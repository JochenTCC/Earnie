...
| **8541** | **Produktion — EHAL Loxone request HTTP** | Synology NAS, LoxBerry, Proxmox LXC | Daemon im selben Container (`Earnie_Request_Optimize` / `/alive`) | LAN: `http://<host>:8541` (nicht Streamlit) | Compose: `8541:8541` neben Streamlit; auch LoxBerry-Plugin [`packaging/loxberry/data/docker/docker-compose.yml`](../../packaging/loxberry/data/docker/docker-compose.yml); Config `system.ehal_loxone_http_port` (Default 8541); VO `VO_Earnie_Status.xml` |
