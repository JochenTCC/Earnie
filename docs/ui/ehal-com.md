...
| Setpoints (Limits)    | `set_evcs_max_current`          | nein*    | **A**; nicht-negativer Betrag (Max. EV-Ladestrom)                                |
| Setpoints (erweitert) | `set_ess_mode`                  | nein*    | ESS-Modus / Steuerbefehl (z. B. Huawei)                                          |
| Setpoints (erweitert) | `set_evcs_current`              | nein*    | **A**; EV-Lade-Sollstrom                                                         |
| Setpoints (erweitert) | `set_evcs_mode`                 | nein*    | Enum: `pv` \| `now`                                                              |
| Capability Flags      | `supports_ess_write`            | ja       | boolean; ESS-Limit-Setpoints dürfen geschrieben werden                           |
| Capability Flags      | `supports_evcs_current`         | ja       | boolean; `set_evcs_max_current` / EV-Strom-Setpoints dürfen geschrieben werden   |
...
| EV Nennstrom (lesbar)          | Messwert    | `sens_evcs_nominal_current`              |                                              |                                  | EVCS Reg. **5017** Max. Ladestrom (A; Leistung = f(A, Phasen, V))      | ersetzt `charging_schedule.loxone.nominal_power_kw_name`   |
| EV Nennleistung / Max-Leistung | Messwert    | TO BE REMOVED                            |                                              |                                  | (ersetzt durch `sens_evcs_nominal_current`)                            | `charging_schedule.loxone.nominal_power_kw_name`           |
...
| EV Lade-Soll-Strom             | Steuerwert  | `set_evcs_current`                       |                                              |                                  |                                                                        | `ehal_bindings.set_evcs_current`                           |
| EV Lademodus                   | Steuerwert  | `set_evcs_mode` (`pv` \| `now`)          |                                              | EVCC-Modus=minpv/pv bzw. now     | EVCS Mode Auto (**5009**=1) / Enable (**5010**)                        | `ehal_bindings.set_evcs_mode`                              |
...
3. **Human-in-the-Loop** — Entity wählen, EHAL-Felder zuweisen; darunter **Event-Trigger** je Entity (`id`, `ehal_field`, `signal_type`, `on_change`, `label`). Die Merker-Adresse kommt aus dem Binding des gewählten Feldes.
