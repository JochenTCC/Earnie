...
**Sonderfälle:** `Netzbezug` und `Geplante Batterie-Aktion` sind vorzeichenkodiert (Bezug/Laden positiv, Einspeisung/Entladen negativ). Fehlt eine explizite Einspeisung in der Zeile, wird PV-Überschuss (`offset_kw > 0`) als gedämpfte PV-Einspeisung gezeichnet. **Grauer Bereich (Produktiv-Log):** PV, Last, Netz, Batterieflüsse und **Flex-Verbraucher** nutzen Ist-Werte aus `consumption_snapshot` / `flex_live_kw` — die Aufteilung Laden/Einspeisung leitet sich aus gemessener Batterieleistung (`Ist Batterie-Leistung (kW)`) ab, nicht aus `battery_plan_kw`. Widerspricht das momentane Ist-Vorzeichen dem SoC-Schritt zum nächsten Log-Slot (z. B. Entladebalken bei steigendem SoC), wird die Batterieleistung für die Balken aus dem SoC-Delta abgeleitet — Punktproben können innerhalb der Viertelstunde drehen. **MILP/neutral/grün:** geplanter Batteriewert; am oberen/unteren SoC-Limit wird geplanter Lade-/Entladeanteil nicht gezeichnet, Überschuss erscheint als PV-Einspeisung bzw. Netzbezug. Im neutralen MILP-Bereich wird `Netzbezug` nach Live-Overlay aus Last, Flex, PV und Batterie neu abgeleitet.
