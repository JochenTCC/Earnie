...
Known residual gap (documented, not fixed): a "mature" config that never had a backend configured can still reach Optimierer-Dienst/EHAL-Com outside the onboarding path — tracked as a follow-up (see dev-plan doc §5 and the spawned task suggestion "Close SB-bypass gap for mature configs without a backend").
