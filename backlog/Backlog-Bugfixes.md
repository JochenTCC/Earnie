...
- [ ] Pool/SwimSpa temperatures missing in EHAL-Com Live-Lesen — thermal_rc rows now include `sens_temperature_water` / Soll / Toleranz / `sens_heating_active` (`ehal_debug_mapping` + `loxone_connectivity`); tests in `test_loxone_connectivity.py` / `test_loxone_debug.py`. Live acceptance pending.
