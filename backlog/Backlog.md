...
- [ ] Build Smoke Test for Integration test / EHAL compatibility fixtures (see `Earnie-Projekt/Entwicklungsplan/Earnie-HA-Kompatibilitaetstests-Entwicklungsdokument.md`) — fixture-based testing with simulated ("faked") device entity signatures instead of real hardware, to test EHAL discovery/mapping against different HA integration shapes. Separate from Add-on M3 (archived; Supervisor persistence walkthrough in [docs/einrichtung/homeassistant-addon-testumgebung.md](../docs/einrichtung/homeassistant-addon-testumgebung.md)). Do this before Phase 4 so later feature work doesn't silently regress discovery/mapping for configs beyond the one real dogfooding instance
  - **Erweiterung (2026-09-21, Anforderungsklärung mit Jochen):** Scope erweitert um einen dynamischen, geschlossenen Regelkreis — Earnie-Setpoints wirken über einen neuen Physik-"State-Stepper" auf den nächsten simulierten Zustand zurück, statt nur statischer Snapshots. Geht über die ursprüngliche Abgrenzung des Entwicklungsdokuments hinaus (Regelkreis war dort explizit ausgeschlossen); die MILP-Entscheidungsqualität (Track-A-Business-Case, `simulation/engine.py`) bleibt weiterhin außerhalb des Scopes. Details/Architektur: Entwicklungsdokument Abschnitt 3
  - [ ] Spike: State-Stepper-API (State + Δt + Setpoints → nächster State) aus vorhandenen Physik-Primitiven extrahieren (Thermal-RC-Modell aus `house_config/thermal_rc_profile.py`, Batteriemodell, PV-Ertragsmodell) — **nicht** `simulation/engine.py::run_simulation()` wiederverwenden (das ist ein MILP-Backtesting-Harness mit Historien-Replay und erneutem Optimizer-Lauf pro Anker-Fenster, kein steuerbarer Live-Stepper)
  - [ ] Spike: prüfen, ob `main.py`s Optimierungs-/Polling-Takt eine injizierbare simulierte Uhrzeit unterstützt (nötig für den beschleunigten CI-Zeittakt) oder ob dafür ein kleiner Hook ergänzt werden muss
  - [ ] Referenz-Archetyp (aus Phase-1-Fixture) an Live-Physik koppeln statt Platzhalterwerten; Entity-Archetyp-Schicht bleibt einzige Quelle für Entity-Struktur-Varianz (keine separate, parallel gepflegte Struktur)
  - [ ] Mock-HA-REST-Server (`/api/states`, `/api/services/*`) für schnelle/CI-taugliche Läufe (beschleunigter Zeittakt); echte HAOS-Instanz (`ha_lab/haos-docker`) folgt später für Echtzeit-Dogfooding/UX-Tests
  - [ ] Config-Input zunächst nur Earnies eigene `house_config` (batteries/pv_systems/tariffs/profile); externe HA-Diagnose-Exports als zusätzliche Archetyp-Quelle bleiben optional/später (Support-Workflow, Entwicklungsdokument Abschnitt 4)
  - [ ] Zugriffsmodell: lokales CLI/Docker-Tool (analog `ha_lab/`) für andere User-Configs, kein gehostetes Multi-User-Tool in dieser Phase
