"""Switch platform for earnie_house_sim (negative write-domain case)."""
...
async def async_setup_entry(
    ...
    entities: list[HouseSimSwitch] = []
    for item in coordinator.package.entities:
        eid = str(item.get("entity_id") or "")
        if not eid.startswith("switch."):
            continue
        entities.append(HouseSimSwitch(coordinator, entry.entry_id, item))
    async_add_entities(entities)


class HouseSimSwitch(CoordinatorEntity[HouseSimCoordinator], SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        self.coordinator.set_switch(self._ha_id, True)
        await self.coordinator.async_request_refresh()

    async def async_turn_off(self, **kwargs: Any) -> None:
        self.coordinator.set_switch(self._ha_id, False)
        await self.coordinator.async_request_refresh()
