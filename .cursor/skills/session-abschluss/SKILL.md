...
- Pre-commit (`.githooks/pre-commit`) **skips pytest** when every staged path is docs/Markdown, **PNG**, Cursor, GitHub, or `.githooks/` (including Git C-quoted non-ASCII names such as `"docs/assets/…png"`). Mixed code+docs still runs the full suite via `python -m scripts.run_pytest` (native). Do **not** pass `--no-verify` to skip tests on code commits.
